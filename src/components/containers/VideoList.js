...
const VideoList = () => {
	...
	useEffect(() => {  /* buscando a playlist de vídeos */		
		...
		async function fetchPlaylist(){
			try{
				const requestPlayList =  await axios.get(`https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet%2C%20contentDetails&maxResults=50&playlistId=${playlistId}&key=${ApiKey}`)
		...
		async function fetchPlaylistTitle(){
			try{
				const requestPlayListTitle =  await axios.get(`https://youtube.googleapis.com/youtube/v3/playlists?part=snippet&id=${playlistId}&key=${ApiKey}`)

				/* setYoutubePlaylist(request.data.items.contentDetails.videoId) */
				setYoutubePlaylistTitle(requestPlayListTitle.data.items[0].snippet.localized.title)				
	...
	useEffect( () => {   /* buscando informações de cada vídeo da lista */	
		...
		videoIdList.map((vidInfo) =>{
			...
			async function fetchVideosInfo(){
				try{
					const requestVideoInfos =  await axios.get(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2C%20contentDetails%2C%20player&id=${vidInfo}&key=${ApiKey}`)
					/* setYoutubePlaylist(request.data.items.contentDetails.videoId) */
